...
async function handler(req: AuthenticatedRequest, res: NextApiResponse): Promise<void> {
  ...
  const result = await pool.query(
    `SELECT d.title, v.file_path, v.mime_type, v.original_file_name
     FROM documents d
  ...
  if (!document?.file_path || !previewMimeTypes.has(document.mime_type)) {
    res.status(415).json({ success: false, error: 'Preview tidak tersedia untuk file ini' });
    return;
  }
